...
# import groundstationclient.config
import groundstationclient.radio.flowgraphs as flowgraphs
from groundstationclient import settings
# from groundstationclient.artifacts import Artifacts
from groundstationclient.observer.worker import WorkerFreq, WorkerTrack

# import requests

# from groundstationclient.scheduler import SCHEDULER
# from groundstationclient.waterfall import EmptyArrayError, Waterfall
...
def rx_device_for_frequency(rx_device_spec, frequency):
    ...
    for range_device in devices:
        try:
            ...
            raise ValueError('Invalid device entry in GROUND_STATION_CLIENT_SOAPY_RX_DEVICE: '
        ...
        try:
            ...
            raise ValueError('Invalid frequency range entry in '
                             'GROUND_STATION_CLIENT_SOAPY_RX_DEVICE: expected format '
                             '<min. freq.>-<max. freq.>; got %r' % freq_range) from exc
        if float(f_min_mhz) <= (frequency / 10**6) <= float(f_max_mhz):
            return device
    raise LookupError(
        'No device found for %.2f MHz in GROUND_STATION_CLIENT_SOAPY_RX_DEVICE = %r' %
...
#     if settings.GROUND_STATION_CLIENT_KEEP_ARTIFACTS:
...
#     :type filename: str
#     """
#     with open(os.path.join(settings.GROUND_STATION_CLIENT_ARTIFACTS_OUTPUT_PATH, filename), 'wb') as f_out:
...
class Observer(object):
    ...
    def __init__(self):
        self.location = {
            ...
            'elev': settings.GROUND_STATION_CLIENT_STATION_ELEV,
        }
        self.rot_port = settings.GROUND_STATION_CLIENT_ROT_PORT
        self.rig_ip = settings.GROUND_STATION_CLIENT_RIG_IP
        self.rig_port = settings.GROUND_STATION_CLIENT_RIG_PORT
        ...
        # new
        self.bw = None
        self.enable_iq_dump = None
        self.downlink_destination_ips = None
        self.uplink_origin_ip = None
    ...
    def setup(
        ...
        * returns False if issue is encountered
        ...
        self.rx_device = rx_device_for_frequency(settings.GROUND_STATION_CLIENT_SOAPY_RX_DEVICE,
                                                 frequency)
        # self.baud = baud
        # self.mode = mode
        ...
        # self.observation_raw_file = '{0}/{1}_{2}_{3}.{4}'.format(settings.GROUND_STATION_CLIENT_OUTPUT_PATH,
        #                                                          not_completed_prefix,
        #                                                          self.observation_id,
        #                                                          self.timestamp,
        #                                                          raw_file_extension)
        # self.observation_ogg_file = '{0}/{1}_{2}_{3}.{4}'.format(settings.GROUND_STATION_CLIENT_OUTPUT_PATH,
        #                                                          completed_prefix,
        #                                                          self.observation_id,
        #                                                          self.timestamp,
        #                                                          encoded_file_extension)
        # self.observation_waterfall_file = '{0}/{1}_{2}_{3}.{4}'.format(
        #     settings.GROUND_STATION_CLIENT_OUTPUT_PATH, receiving_waterfall_prefix, self.observation_id,
        #     self.timestamp, waterfall_file_extension)
        # self.observation_waterfall_png = '{0}/{1}_{2}_{3}.{4}'.format(
        #     settings.GROUND_STATION_CLIENT_OUTPUT_PATH, waterfall_prefix, self.observation_id, self.timestamp,
        #     'png')
        # self.observation_receiving_decoded_data = '{0}/{1}_{2}_{3}.{4}'.format(
        #     settings.GROUND_STATION_CLIENT_OUTPUT_PATH, receiving_decoded_data_prefix, self.observation_id,
        #     self.timestamp, 'png')
        # self.observation_done_decoded_data = '{0}/{1}_{2}_{3}.{4}'.format(
        #     settings.GROUND_STATION_CLIENT_OUTPUT_PATH, decoded_data_prefix, self.observation_id, self.timestamp,
        #     'png')
        # self.observation_decoded_data = '{0}/{1}_{2}'.format(settings.GROUND_STATION_CLIENT_OUTPUT_PATH,
        #                                                      decoded_data_prefix,
        #                                                      self.observation_id)
        self.bw = bw
        self.enable_iq_dump = save_iq_files
    ...
    def observe(self):  # pylint: disable=R0915
        ...
        if settings.GROUND_STATION_CLIENT_PRE_OBSERVATION_SCRIPT is not None:
            LOGGER.info('Executing pre-observation script.')

            # script_name = flowgraphs.GROUND_STATION_CLIENT_FLOWGRAPH_MODES[
            #     flowgraphs.GROUND_STATION_CLIENT_FLOWGRAPH_MODE_DEFAULT]['script_name']
            # if self.mode in flowgraphs.GROUND_STATION_CLIENT_FLOWGRAPH_MODES:
            #     script_name = flowgraphs.GROUND_STATION_CLIENT_FLOWGRAPH_MODES[self.mode]['script_name']
            ...
            pre_script = []
            for arg in shlex.split(settings.GROUND_STATION_CLIENT_PRE_OBSERVATION_SCRIPT):
                repl_arg = arg
        ...
        if settings.GROUND_STATION_CLIENT_ROT_ENABLED:
        ...
        # flowgraph = groundstationclient.radio.flowgraphs.Flowgraph(
        flowgraph = flowgraphs.Flowgraph(
            self.rx_device,
            settings.GROUND_STATION_CLIENT_RX_SAMP_RATE,
            self.frequency,
            self.bw,
            self.enable_iq_dump,
            self.downlink_destination_ips,
            self.uplink_origin_ip,
            # self.mode,
            # self.baud,
            # {
        ...
        #     if settings.GROUND_STATION_CLIENT_REMOVE_RAW_FILES:
        ...
        # client_metadata['latitude'] = settings.GROUND_STATION_CLIENT_STATION_LAT
        # client_metadata['longitude'] = settings.GROUND_STATION_CLIENT_STATION_LON
        # client_metadata['elevation'] = settings.GROUND_STATION_CLIENT_STATION_ELEV
        ...
        #                         headers=headers,
        #                         data={
        #                             'client_version': groundstationclient.config.VERSION,
    ...
    def poll_gnu_proc_status(self, flowgraph):
        ...
        if settings.GROUND_STATION_CLIENT_POST_OBSERVATION_SCRIPT is not None:
            ...
            post_script = []
            for arg in shlex.split(settings.GROUND_STATION_CLIENT_POST_OBSERVATION_SCRIPT):
                repl_arg = arg
                for key, val in replacements:
                    repl_arg = repl_arg.replace(key, val)
    ...
    #     vmin = None
    #     vmax = None
    #     if not settings.GROUND_STATION_CLIENT_WATERFALL_AUTORANGE:
    #         vmin = settings.GROUND_STATION_CLIENT_WATERFALL_MIN_VALUE
    #         vmax = settings.GROUND_STATION_CLIENT_WATERFALL_MAX_VALUE
