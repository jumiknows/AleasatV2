...
from alea.ttc.spacecraft.gps import gps_utils

from alea.sa.backend import ttcqt
from alea.sa.ui.control_panels import gps_panel_ui
...
class GPSPanel(QtWidgets.QWidget, gps_panel_ui.Ui_GpsPanel):
    def __init__(self, obc_provider: ttcqt.OBCTTCInterfaceProvider, parent=None):
        ...
        self.gps_set_nmea_settings_gsa_line: QtWidgets.QLineEdit
        self.gps_get_nmea_settings_gsa_line: QtWidgets.QLineEdit
        ...
        self.gps_software_ver_mode_edit: QtWidgets.QLabel
        ...
        self.gps_get_software_ver_btn.clicked.connect(self.handle_get_software_ver)
        self.gps_get_pos_update_btn.clicked.connect(self.handle_get_position_update_rate)
        self.gps_set_pos_update_btn.clicked.connect(self.handle_set_position_update_rate)
        self.gps_set_nav_interval_btn.clicked.connect(self.handle_set_navigation_msg_int)
        self.gps_get_nmea_settings_btn.clicked.connect(self.handle_get_nmea_settings)
        self.gps_set_nmea_settings_btn.clicked.connect(self.handle_set_nmea_settings)
    ...
    def _capture_pwr_mode_callback(self, resp: cmd_sys.OBCResponse | Exception):
        if not ttc_utils.check_cmd_sys_resp(resp, "gps_in_power_save_mode"):
            return
    ...
    def handle_set_pwr_mode(self):
        ...
        write_mode = gps_utils.WriteSetting.get(
    ...
    def _capture_software_crc_callback(self, resp: cmd_sys.OBCResponse | Exception):
        if not ttc_utils.check_cmd_sys_resp(resp, "software_crc"):
            return
    ...
    def handle_restart(self):
        restart_mode = gps_utils.RestartMode.get(
    ...
    def _capture_software_ver_callback(self, resp: cmd_sys.OBCResponse | Exception):
        if not ttc_utils.check_cmd_sys_resp(resp, "gps_software_version"):
            return
    ...
    def _capture_position_update_rate(self, resp: cmd_sys.OBCResponse | Exception):
        if not ttc_utils.check_cmd_sys_resp(resp, "update_rate"):
            return
    ...
    def _capture_nmea_settings(self, resp: cmd_sys.OBCResponse | Exception):
        if not ttc_utils.check_cmd_sys_resp(resp, "gga_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "gsa_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "gsv_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "gll_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "rmc_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "vtg_interval") or \
                not ttc_utils.check_cmd_sys_resp(resp, "zda_interval"):
            return

        self.gps_get_nmea_settings_gga_line.setText(str(resp.data["gga_interval"]))
        self.gps_get_nmea_settings_gsa_line.setText(str(resp.data["gsa_interval"]))
        self.gps_get_nmea_settings_gsv_line.setText(str(resp.data["gsv_interval"]))
        self.gps_get_nmea_settings_gll_line.setText(str(resp.data["gll_interval"]))
        self.gps_get_nmea_settings_rmc_line.setText(str(resp.data["rmc_interval"]))
        self.gps_get_nmea_settings_vtg_line.setText(str(resp.data["vtg_interval"]))
        self.gps_get_nmea_settings_zda_line.setText(str(resp.data["zda_interval"]))
