...
class CameraPanel(QtWidgets.QWidget, camera_panel_ui.Ui_CameraPanel):
    ...
    def _capture_callback(self, resp: Union[cmd_sys.resp.OBCResponse, exc.OBCError]):
        ...
        if resp.data["image_size"] <= 0:
            console_utils.print_err(f"Invalid image size: {resp.data['image_size']}")
            return
    ...
    def handle_sreg_read(self):
        try:
            ...
            console_utils.print_err("Error parsing addr field")

    def _sreg_read_callback(self, resp: Union[cmd_sys.resp.OBCResponse, exc.OBCError]):
        if not obc_utils.check_cmd_sys_resp(resp, "ov5642_err", "data"):
            return
    ...
    def handle_sreg_write(self):
        try:
            ...
            console_utils.print_err("Error parsing camera sensor addr/data fields")
    ...
    def handle_res_set(self):
        try:
            ...
            console_utils.print_err("Error parsing camera resolution fields")
    ...
    def handle_wb_set(self):
        try:
            ...
            console_utils.print_err("Error parsing camera white balance fields")
