...
/**
 * @file dma.h
 * @brief Configuration file for Direct Memory Access (DMA) routines
...
/**
 *@brief The DMA channel types used
 */
typedef enum {
...
// Macro to slice the high byte out of a 2 byte int16_t
#define DMA_ADDR_HIGH(p) (((uint16_t) (__xdata uint8_t *) (p)) >> 8)

// Macro to slice the low byte out of a 2 byte int16_t
...
// Macro to set the DMA interrupt on
// and to arm the channel to start transfer
// If we do runtime configuration of DMA channels then this
// should have 9 NOP instructions afterwards
...
// Make DMA configurations global scope
