...
"""
class WmmTest(obc_test.OBCTest):
    ...
    def test_wmm_init(self):
        self.obc.send_cmd("WMM_INIT")
        resp = self.obc.send_cmd("WMM_IS_INIT")
    ...
    def test_wmm_no_init_call_mag_ref(self):
        resp = self.obc.send_cmd("WMM_GET_MAG_REF", 1, 1, 1, self.d1.year-2000, self.d1.month, self.d1.day)
    ...
    def test_mag_ref_similarity(self, date, alt, lat, lon):
        ...
        if not (self.obc.send_cmd("WMM_IS_INIT")).data["is_init"]:
            self.obc.send_cmd("WMM_INIT")
        ...
        resp = self.obc.send_cmd("WMM_GET_MAG_REF", lon, lat, alt, date.year-2000, date.month, date.day)
        self.assertTrue(resp.is_success)
