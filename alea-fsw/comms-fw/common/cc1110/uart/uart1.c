...
static uint8_t __xdata rx_buffer[UART1_RX_BUFFERS][LOCAL_PACKET_MAX_LEN];
...
void uart1_init(void) {
    ...
    // Bit 5 sets bit order (LSBit/MSBit) of byte for (0/1), set to 0
    // Bits 6 and 7 are not used (they are SPI settings)
    // For CONFIG_UART1_CGR, 12 = 115200 baud, 14 = 460800 baud
    ...
    // Set UART1 RX interrupts to level 1 priority
...
#ifndef BOOTLOADER
/**
 * @brief Check if all UART buffers are empty
 *
 * @return If all buffers are empty or not
 */
__bit uart1_buffers_empty(void) {
    __bit ret;
    uint8_t i;
...
inline void uart1_put(char c) {
    // Write character to internal data buffer
    U1DBUF = c;
...
void uart1_send_message(const __xdata uint8_t *msg, uint8_t len) {
    // Clear any pending TX interrupt request
    U1CSR &= ~U1CSR_TX_BYTE;
...
/**
 * @brief Send message to destination over UART1
 *
 * @param dest
 * @param cmd
 * @param data
 * @param data_len
 */
void send_command_uart1(hwid_t dest, radio_msg_no_t cmd, const uint8_t *data, uint8_t data_len) {
    ...
    uart1_send_message((__xdata uint8_t *) &print_buf, len);
}

// void uart1_dbg(const __xdata uint8_t *msg, uint8_t len) {
//     uart1_put(0xFD);
//     while (len--) {
//         uart1_put(*(msg++));
//     }
    ...
    switch (rx_esp_state) {
        ...
        case wait_for_length:
            if (c > LOCAL_PACKET_MAX_LEN || c < 1) {
                ...
                // Find a free buffer
                rx_active_buffer = 0;
                while ((rx_active_buffer < UART1_RX_BUFFERS) && rx_buffer_ready[rx_active_buffer]) {
                    rx_active_buffer++;
                }
