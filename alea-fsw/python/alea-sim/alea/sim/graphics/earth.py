...
class Earth3D:
    def __init__(self, scale: float = 1.0, base_color: str = "lightskyblue",
                 plot_grid: bool = True, plot_equator: bool = True, plot_prime_meridian: bool = False):
        self._geometry = shapes.WGS84Earth(scale)
        self._base = Spheroid(self._geometry, color=base_color, wireframe=False, phi_step_deg=5, theta_step_deg=5, zorder=0)

        self._plot_grid = plot_grid
        if plot_grid:
            self._grid = Spheroid(self._geometry, color="lightgrey", wireframe=True, phi_step_deg=5, theta_step_deg=5, zorder=1, linewidth=0.1)
    ...
    def plot(self, ax: mplot3d.Axes3D) -> list[art3d.artist.Artist]:
        ...
        if self._plot_equator:
            ...
            self._equator, = ax.plot(x, y, z, color="red", linewidth=0.75, zorder=2)
            artists.append(self._equator)

        # Plot prime meridian
        if self._plot_prime_meridian:
            x, y, z = self._geometry.cartesian_surface_arc_polar(phi_deg=0, num_points=100)
            self._prime_meridian, = ax.plot(x, y, z, color="red", linewidth=0.75, zorder=2)
