...
class FlashTest(OBCTestClass.OBCTest):
    ...
    def test_flash_rw_80(self):
        self.obc.send_q.put("0 test_flash_rw * * 80 64")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if passKeyword in msg.print():
                return
    ...
    def test_flash_rw_last_64(self):
        self.obc.send_q.put("0 test_flash_rw * * 1FFFFC0 64")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if passKeyword in msg.print():
                return
    ...
    def test_flash_rw_addr_bad_len_32(self):
        self.obc.send_q.put("0 test_flash_rw * * 0 32")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if failKeyword in msg.print():
                return
            elif passKeyword in msg.print():
                self.fail("Pass keyword observed: test failed.")

    # FAIL path 1: len = 128
    #    Len =/= 64 bytes
    @timeout.timeout(10)
    def test_flash_rw_addr_bad_len_128(self):
        self.obc.send_q.put("0 test_flash_rw * * 0 128")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if failKeyword in msg.print():
                return
    ...
    def test_flash_rw_addr_too_large(self):
        self.obc.send_q.put("0 test_flash_rw * * 02000000 64")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if failKeyword in msg.print():
                return
    ...
    def test_flash_rw_addr_unaligned_1(self):
        self.obc.send_q.put("0 test_flash_rw * * 1 64")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if failKeyword in msg.print():
                return
    ...
    def test_flash_rw_addr_unaligned_1fffc01(self):
        self.obc.send_q.put("0 test_flash_rw * * 01FFFC01 64")
        while 1:
            msg = self.obc.recv_q.get(block=True)
            if failKeyword in msg.print():
                return
