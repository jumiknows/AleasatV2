...
class OBCTaskSpecs:
    ...
    def _validate(self):
        ...
        Raises:
            exc.OBCTaskSpecError: If a duplicate ID exists in the list of specifications.
        """
        ...
        for spec in self._specs:
            ...
            if spec.id in specs_by_id:
                raise exc.OBCTaskSpecError(f"Duplicate ID ({spec.id}) for {specs_by_id[spec.id].name} and {spec.name}")
            ...
            if len(spec.name) >= self.MAX_TASK_NAME_LEN:
                raise exc.OBCTaskSpecError(f"Task name too long: \"{spec.name}\". Must be < {self.MAX_TASK_NAME_LEN} characters")

        max_id = self.max_id
        if max_id > self.MAX_ALLOWED_ID:
            raise exc.OBCTaskSpecError(f"Maximum allowed task ID exceeded by {specs_by_id[max_id].name} task with ID: {max_id}")
    ...
    def get(self, name: str = None, id: int = None) -> OBCTaskSpec:
        ...
        Raises:
            exc.OBCTaskNotFoundError: If there is no spec with the given name or ID.
        ...
        if name is not None:
            ...
            try:
                ...
                raise exc.OBCTaskNotFoundError(f"No task with name: {name}")

        if id is not None:
            try:
                return next(spec for spec in self._specs if spec.id == id)
            except StopIteration:
                raise exc.OBCTaskNotFoundError(f"No task with ID: {id}")

    def __iter__(self) -> Iterator[OBCTaskSpec]:
        return self._specs.__iter__()

    def __getitem__(self, val: Union[int, slice]) -> Union[OBCTaskSpec, List[OBCTaskSpec]]:
    ...
    @classmethod
    def from_json(cls, *json_blobs) -> "OBCTaskSpecs":
        """Parses a collection of JSON blobs (parsed by `json.load`) containing task specifications.
