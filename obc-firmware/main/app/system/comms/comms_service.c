...
static void dispatch_command_handler(
    comms_command_t* cmd_in,
    comms_command_t* cmd_resp
);

static comms_err_t session_send_cmd(
...
static void session_notify_event(
    session_t* sess,
    comms_event_id_t ev_id,
    void* arg
);

static void session_notify_all(
    comms_event_id_t ev_id,
    void* arg
);

static void session_handle_command_failure(
    session_t* sess
);

static void session_handle_command_response(
    session_t* sess,
    comms_command_t* cmd
);

static void session_handle_retransmission(
    session_t* sess
);
...
void comms_service_create_infra(void)
{
    ...
    cmd_timer = xTimerCreateStatic("comms_service_timer",
                                   pdMS_TO_TICKS(COMMS_CMD_PKT_TIMER_PERIOD_MS),
                                   pdFALSE,
                                   0,
                                   &handle_timer_expiry,
                                   &timer_buffer);
...
comms_err_t comms_send_command(
    ...
    do {
        if((MAX_NUM_SESSIONS <= (uint8_t)session_handle) ||
           (COMMS_CMD_MAX <= (uint8_t)cmd_id)) {
            err = COMMS_ERR_INVALID_ARG;
            break;
        }
        ...
        if((COMMS_CMD_APP_DATA == cmd_id) &&
           (((COMMS_MAX_CMD_PAYLOAD_NUM_BYTES * RETRANSMIT_NUM_PKT) < payload_len))) {
            err = COMMS_ERR_BUF_OVERFLOW;
            break;
        }
        ...
        err = session_send_cmd(session_handle, command_table[cmd_id],
                               payload, payload_len);
        if(err != COMMS_SUCCESS) {
            xSemaphoreGive(cmd_sema);
        }
...
void comms_service_packet_input(
    ...
    if(cmd_in->header.is_response == 0) {
...
static void session_notify_all(
    ...
    for(i = 0; i < session_count; i++) {
        session_notify_event(&sessions[i], ev_id, arg);
    }
}
...
static void session_handle_command_response(
    ...
    if((cmd->header.seqnum == cmd_buf.header.seqnum) &&
       (cmd->header.command != COMMS_COMMON_MSG_NACK) && (cmd->header.command != COMMS_BOOTLOADER_MSG_NACK)) {
