...
from alea.sa.ui import obc_cmd_resp_sched_panel_ui
...
class OBCCmdRespSchedPanel(QtWidgets.QWidget, obc_cmd_resp_sched_panel_ui.Ui_OBCCmdRespSchedPanel, obc_cmd_resp_base_panel.OBCCmdRespBasePanel):
    ...
    @QtCore.pyqtSlot(cmd_sys.OBCPendingResponse)
    def handle_resp_recvd(self, resp: cmd_sys.OBCPendingResponse):
        try:
            row_idx = self._pending_cmds[resp.sched_resp.cmd_inst_id]
        except KeyError:
            console.print_err(f"Unexpected response: cmd_inst_id = {resp.sched_resp.cmd_inst_id}")
            return
