...
class OBCTaskTraceError(Exception):
    pass
...
class OBCRTOSTrace:
    ...
    def __getitem__(self, val: int | slice) -> OBCRTOSTraceEntry | list[OBCRTOSTraceEntry]:
        ...
        for entry in self._entries:
            if entry.event == OBCRTOSTraceEvent.TASK_SWITCHED_IN:
                if last_event == OBCRTOSTraceEvent.TASK_SWITCHED_IN:
                    raise OBCTaskTraceError("Consecutive TASK_SWITCHED_IN events")
                ...
                if last_event == OBCRTOSTraceEvent.TASK_SWITCHED_OUT:
                    raise OBCTaskTraceError("Consecutive TASK_SWITCHED_OUT events")
                elif last_event == None:
                    continue

                if entry.task_id != switch_in_task:
                    raise OBCTaskTraceError("Task changed without event")
                    break
