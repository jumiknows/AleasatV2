...
uint32_t rtos_stream_read_msg(void *handle, uint8_t *buf, uint32_t buf_len, uint32_t timeout, uint32_t *timeout_left) {
    ...
    if (stream->msg_buf != NULL) {
        ...
        msg_len = xMessageBufferReceive(stream->msg_buf, buf, buf_len, timeout);
...
uint32_t rtos_stream_write_msg(void *handle, const uint8_t *data, uint32_t num_bytes, uint32_t timeout, uint32_t *timeout_left) {
    ...
    if (stream->msg_buf != NULL) {
        ...
        if (stream->mutex != NULL) {
            if (!xSemaphoreTake(stream->mutex, timeout)) {
                return 0;
            }
        ...
        bytes_written = xMessageBufferSend(stream->msg_buf, data, num_bytes, timeout_left_internal);
 ...
 * 
 * The timeout_left is calculated as the start subtract from the current tick count,
