...
cmd_sys_err_t cmd_impl_TEST_CAM_CAPTURE(const cmd_sys_cmd_t *cmd, cmd_TEST_CAM_CAPTURE_resp_t *resp, const data_fmt_desc_t *resp_desc,
                                        uint32_t resp_len, uint8_t *buf) {
    ...
    LOG_CAMERA__IMAGE_CAPTURED(err);
    ...
    if (err == ARDUCAM_SUCCESS) {
        ...
        LOG_CAMERA__IMAGE_SIZE(err, img_size);
    ...
    if (err == ARDUCAM_SUCCESS) {
        ...
        while (!done) {
            ...
            if (err != ARDUCAM_SUCCESS) {
                ...
                LOG_CAMERA__ARDUCAM_TRANSFER_ERR(err);
                break;
            }
            ...
            if (data_len > 0) {
                ...
                if (bytes_written != data_len) {
                    LOG_CAMERA__DOWNLINK_TRANSFER_ERR(data_len, bytes_written);
                    break;
                }
            } else {
