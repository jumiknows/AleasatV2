...
/**
 * @file telemetry.c
 * @brief Radio telemetry acquisition and reporting
...
/**
 * @brief Set telemetry fields to intial values
 * @sideeffect Telemetry address space updated with intial values
 */
...
/**
 * @brief Update old telemetry data with new telemetry data
 *
 * @warning PKTCTRL1_NO_APPEND_STATUS will set cs count, rssi, and freqest to 0
 */
void update_telemetry(void) {
    telemetry.reserved = 0;
    __critical {
        telemetry.uptime = uptime;
        telemetry.spi_rx_count = spi_rx_count;
    ...
    // If the telemetry is queried during transmission or reception
    // these fields will be set with that information
    ...
    telemetry.packets_rejected_authent = radio_packets_rejected_authent;
