...
const heading: Array<
  keyof components["schemas"]["GmatOrbitPropagation"]["result"][number]
> = ["utcGregorian", "x", "y", "z"];
...
const parseGmatResult = async (
  ...
  return await tracer.startActiveSpan("parseGmatResult", async (span) => {
    ...
    const result =
      [] as components["schemas"]["GmatOrbitPropagation"]["result"];
    resultCsv
      .split("\n")
      .slice(1, -1)
      .forEach((line) => {
        // @ts-expect-error temp result
        const lineResult: components["schemas"]["GmatOrbitPropagation"]["result"][number] =
          {};
