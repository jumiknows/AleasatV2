...
static obc_cmd_err_t invoke(cmd_t* cmd, action_id_t* action_id, bool immediate, scheduler_submission_source_t source) {
    ...
    uint8_t num_args = cmd->num_of_arguments;
    char* arguments = cmd->arguments;

    if (immediate) {
        err = scheduler_schedule_action_start_now(action_id, source, interval, action, num_args, arguments);
    } else {
        err = scheduler_schedule_action(action_id, source, start_at, interval, action, num_args, arguments);
    }
...
static obc_cmd_err_t parse(cmd_t* cmd, char* input_cmd_str, char** opcode_out) {
    *opcode_out        = NULL; // Set the default output
    const char _space_delim[] = " ";
    char* save_ptr = NULL;
    char* token = orca_strtok_r(input_cmd_str, _space_delim, &save_ptr);
    ...
    token = orca_strtok_r(NULL, _space_delim, &save_ptr);
    if (token == NULL) {
        return OBC_CMD_SYNTAX_ERROR;
    }
    *opcode_out = token; // Update the opcode output with a pointer to what we think is the opcode.
    ...
    token = orca_strtok_r(NULL, " -", &save_ptr);
    if (token == NULL) {
        return OBC_CMD_SYNTAX_ERROR;
    }
    ...
    if (*token == '*') {
        ...
        token = orca_strtok_r(NULL, " -", &save_ptr);
        // Turning off MISRA #1404-D (MISRA-C:2004 12.4/R) since it thinks that
        // the right-hand operand contains side effects.
        OBC_MISRA_CHECK_OFF;
        if ((token == NULL) || (!is_year_valid(val))) {
            return OBC_CMD_SYNTAX_ERROR;
        }
        OBC_MISRA_CHECK_ON;
        ...
        // the right-hand operand contains side effects.
        OBC_MISRA_CHECK_OFF;
        if ((token == NULL) || (!is_month_valid(val))) {
            return OBC_CMD_SYNTAX_ERROR;
        }
        OBC_MISRA_CHECK_ON;
        ...
        // the right-hand operand contains side effects.
        OBC_MISRA_CHECK_OFF;
        if ((token == NULL) || (!is_day_valid(cmd->start_at.year, cmd->start_at.month, val))) {
            return OBC_CMD_SYNTAX_ERROR;
        }
        OBC_MISRA_CHECK_ON;
        ...
        // the right-hand operand contains side effects.
        OBC_MISRA_CHECK_OFF;
        if ((token == NULL) || (!is_hour_valid(val))) {
            return OBC_CMD_SYNTAX_ERROR;
        }
        OBC_MISRA_CHECK_ON;
        ...
        // the right-hand operand contains side effects.
        OBC_MISRA_CHECK_OFF;
        if ((token == NULL) || (!is_minute_valid(val))) {
            return OBC_CMD_SYNTAX_ERROR;
        }
        OBC_MISRA_CHECK_ON;
    ...
    if (token != NULL) {
        cmd->arguments = token;
        cmd->num_of_arguments  = strlen(token);
    } else {
        cmd->arguments = NULL;
        cmd->num_of_arguments  = 0;
    }
