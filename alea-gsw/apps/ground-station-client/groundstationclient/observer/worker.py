...
from groundstationclient.observer.orbital import pinpoint
from groundstationclient.rig import Rig
from groundstationclient.rotator import Rotator
...
class WorkerTrack(Worker):
    ...
    def _communicate_tracking_info(self):
        ...
        """
        rotator = Rotator(
            settings.GROUND_STATION_CLIENT_ROT_MODEL,
            settings.GROUND_STATION_CLIENT_ROT_BAUD,
            settings.GROUND_STATION_CLIENT_ROT_PORT,
        )
        rotator.open()
    ...
    def trackobject(self, *args):
        ...
        if (settings.GROUND_STATION_CLIENT_ROT_FLIP
                and settings.GROUND_STATION_CLIENT_ROT_FLIP_ANGLE):
            ...
            self._flip = (self._midpoint[1] >= settings.GROUND_STATION_CLIENT_ROT_FLIP_ANGLE)
    ...
    def send_to_socket(self, pin, sock):
        ...
        if (abs(azi - cur_azi % 360.0) > settings.GROUND_STATION_CLIENT_ROT_THRESHOLD
                or abs(alt - cur_alt) > settings.GROUND_STATION_CLIENT_ROT_THRESHOLD):
            msg = (azi, alt)
