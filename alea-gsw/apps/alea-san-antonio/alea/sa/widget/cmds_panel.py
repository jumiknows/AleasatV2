...
from alea.sa.ui.control_panels import cmds_panel_ui
...
class CmdsPanel(QtWidgets.QWidget, cmds_panel_ui.Ui_CmdsPanel):
    def __init__(self, obc_provider: ttcqt.OBCTTCInterfaceProvider, parent=None):
        ...
        cmd_names = list(map(lambda spec: spec.name, self.obc.cmd_sys_specs))
    ...
    def handle_cmd_name_changed(self, cmd_name: str):
        try:
            spec = self.obc.cmd_sys_specs.get(cmd_name)
            self.custom_cmd_help_label.setText(str(spec))
        except cmd_sys_spec.OBCCmdSysSpecNotFoundError:
    ...
    def handle_send(self):
        ...
        if self.scheduled_radio_btn.isChecked():
            cmd_date_time = alea_time.ALEADateTime(self.date_time_edit.dateTime().toPyDateTime())
        else:
            cmd_date_time = alea_time.IMMEDIATE

        self.obc.execute(ttcqt.TTCQTRequest(
            lambda obc, cmd_str=cmd_str: obc.send_obc_cmd_str(cmd_str, date_time = cmd_date_time)
        ))

    @QtCore.pyqtSlot()
    def handle_get_fw_info(self):
        self.obc.execute(ttcqt.TTCQTRequest(
            lambda obc: obc.send_obc_cmd("FW_INFO", timeout=1),
            self._fw_info_callback
        ))

    def _fw_info_callback(self, resp: cmd_sys.OBCResponse | Exception):
