...
class EPSFeature(ttc_base.TTCBase):
    """Implementation of eps features of the OBC
    """

    def eps_write(self, id: EPSWriteCmd, state: EPSState, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_WRITE", id, state, timeout=timeout)

    def eps_read(self, id: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_READ_INT", id, timeout=timeout)

    def eps_measure_buses(self, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_MEASURE_BUSES", timeout=timeout)

    def eps_measure_temps(self, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_MEASURE_TEMPS", timeout=timeout)

    def eps_read_counters(self, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_READ_COUNTERS", timeout=timeout)

    def eps_read_status(self, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_EPS_READ_STATUS", timeout=timeout)
