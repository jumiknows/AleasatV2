...
class OBCCameraFeature(obc_base.OBCBase):
    ...
    def camera_read_sensor_reg(self, addr: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.resp.OBCResponse:
        return self.send_cmd("TEST_CAM_RD_SREG", addr, timeout=timeout)

    def camera_write_sensor_regs(self, regs: OV5642.RegisterSet, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.resp.OBCResponse:
    ...
    def camera_set_resolution(self,
                          ...
                          timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.resp.OBCResponse:
        # Read ISP_CONTROL_01
    ...
    def camera_set_white_balance_manual(self, gain_red: int, gain_green: int, gain_blue: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.resp.OBCResponse:
        regs = OV5642.RegisterSet([
    ...
    def camera_set_white_balance_auto_simple(self, top_limit: int, bottom_limit: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.resp.OBCResponse:
        regs = OV5642.RegisterSet([
