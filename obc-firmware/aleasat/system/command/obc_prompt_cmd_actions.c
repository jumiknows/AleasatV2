...
void cmd_ack(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_ACK_CMD, "Ack!");
}
...
void cmd_ping(uint32_t arg_len, void* arg) {
    ...
    prompt_cmd_response(INFO, LOG_PING_CMD, "Ping");
}
...
void cmd_get_board_type(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_BOARD_ID, BOARD_TYPE_MSG);
}
...
void cmd_get_sequence_num(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_SEQ_NUM_CMD, "%d", obc_cmd_next_seq_num());
}
...
void cmd_last_response(uint32_t arg_len, void* arg) {
    log_str(last_logged.log_lvl, last_logged.log_id, "%s", last_logged.string);
}
...
void cmd_temp_reset(uint32_t arg_len, void* arg) {
    reset_temp();
    prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Reset temp sensor");
}
...
void cmd_expander_reset(uint32_t arg_len, void* arg) {
    gpio_expander_reset();
    prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Reset GPIO expander");
}
...
void cmd_i2c_reset(uint32_t arg_len, void* arg) {
    ...
    if (err == I2C_SUCCESS) {
        prompt_cmd_response(INFO, LOG_I2C, "I2C bus reset");
    } else {
        prompt_cmd_response(INFO, LOG_I2C, "I2C reset failed: %d", err);
    }
}
...
void cmd_get_time(uint32_t arg_len, void* arg) {
    ...
    if (err == RTC_NO_ERR) {
        prompt_cmd_response(INFO, LOG_GET_EPOCH_CMD, "%d-%d-%d %d:%d:%d", curr_time.year, curr_time.month, curr_time.day, curr_time.hour, curr_time.minute, curr_time.second);
    } else {
        prompt_cmd_response(ERROR, LOG_GET_EPOCH_CMD, "Cannot read time.");
    }
}
...
void cmd_get_epoch(uint32_t arg_len, void* arg) {
    ...
    if (epoch != -1) {
        prompt_cmd_response(INFO, LOG_GET_EPOCH_CMD, "Epoch: %d", epoch);
    } else {
        prompt_cmd_response(ERROR, LOG_GET_EPOCH_CMD, "Epoch invalid.");
    }
}
...
void cmd_get_epoch_and_time(uint32_t arg_len, void* arg) {
    ...
    rtc_err_t err         = rtc_get_current_time(&curr_time);
    if (err == RTC_NO_ERR) {
        prompt_cmd_response(INFO, LOG_GET_EPOCH_CMD, "%d %d-%d-%d %d:%d:%d", curr_time.epoch, curr_time.year, curr_time.month, curr_time.day, curr_time.hour, curr_time.minute, curr_time.second);
    } else {
        prompt_cmd_response(ERROR, LOG_GET_EPOCH_CMD, "Cannot read time.");
    }
}
...
void cmd_get_backup_epoch(uint32_t arg_len, void* arg) {
    ...
    prompt_cmd_response(INFO, LOG_GET_BACKUP_EPOCH_CMD, "Backup Epoch: %d", backup_epoch);
}
...
void cmd_set_time(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 7) {
        ...
        prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Too few args. Need rtc y m d h m s.");
    }
}
...
void cmd_set_rtc(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 1) {
        ...
        prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Too few args. Need rtc.");
    }
}
...
void cmd_rtc_ts(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 1) {
        ...
        if (ts == -1) {
            log_str(ERROR, LOG_RTC_GENERAL, "Invalid ts arg %s", (char*)arg);
            return;
        }

        if ((ts == 1) || (ts == 0)) {
            rtc_set_ts(ts);
            ts ? prompt_cmd_response(INFO, LOG_RTC_GENERAL, "RTC mode: ts") : prompt_cmd_response(INFO, LOG_RTC_GENERAL, "RTC mode: reg");
        } else {
            prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Invalid ts arg %s", (const char*)arg);
        }
    } else {
        prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Usage: rtc_ts 1");
    }
}
...
void cmd_get_count(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Count: %d", demo_counter);
}
...
void cmd_run_hang_task(uint32_t arg_len, void* arg) {
    if (set_hang_task_status_alive()) {
        prompt_cmd_response(INFO, LOG_HANG_TASK, "Hanging a test task.");
    } else {
        prompt_cmd_response(ERROR, LOG_HANG_TASK, "Error when hanging test task.");
    }
}
...
void cmd_echo_to_log(uint32_t arg_len, void* arg) {
    ...
    uint8_t num_args   = obc_cmd_read_str_arguments(arg, 1, arguments);
    if (num_args == 1) {
        prompt_cmd_response(INFO, LOG_ECHO_CMD, arguments[0]);
    } else {
        prompt_cmd_response(ERROR, LOG_ECHO_CMD, "Can only echo single word.");
    }
}
...
void cmd_LP_on(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Enabled low power!");
    idle_sleep_on();
}
...
void cmd_LP_off(uint32_t arg_len, void* arg) {
    prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Disabled low power!");
    idle_sleep_off();
}
...
void cmd_rtos_suspend(uint32_t arg_len, void* arg) {
    if (suspend_task((char*)arg, true) != OBC_RTOS_OK) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Invalid task name");
    }
}
/**
...
void cmd_rtos_period(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 0) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Task name not found");
        return;
    }
    uint8_t id      = 0;
    char* task_name = arguments[0];
    if (get_task_id_from_name(task_name, &id) != OBC_RTOS_OK) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Task ID %d not found", id);
        return;
    }
    task_period_t period_ms = 0;
    if (num_args == 1) { // only have task name in cmd. return current period
        prompt_cmd_response(INFO, LOG_OBC_TASK, "Task %s per %d", task_name, period_ms);
        return;
    } else if (num_args != 2) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "1 or 2 arguments required");
        return;
    }
    char* task_period = arguments[1];
    if (task_period == NULL) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Task period not found");
        return;
    }

    period_ms = cseq_to_num(task_period);
    if (period_ms == -1) {
    	log_str(ERROR, LOG_OBC_TASK, "Invalid period %s", (char*)arg);
        return;
    }

    if (period_ms <= 0) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Task period not valid");
        return;
    }
    ...
    if (set_task_period(id, period_ms) != OBC_RTOS_OK) {
        prompt_cmd_response(ERROR, LOG_OBC_TASK, "Task is not periodic");
    }
}
...
void cmd_get_temperature(uint32_t arg_len, void* arg) {
    ...
    if (read_temp(&temp) == TEMP_SUCCESS) {
        prompt_cmd_response(INFO, LOG_TEMPERATURE, "Temperature: %d C", temp);
    } else {
        prompt_cmd_response(ERROR, LOG_TEMPERATURE, "Temperature read error.");
    }
}
...
void cmd_get_period(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 0) {
        ...
        if (action_id == -1) {
            log_str(ERROR, LOG_RTC_GENERAL, "Invalid action id: %s", (char*)arg);
            return;
       	}
        ...
        if (result == SCHEDULER_SUCCESS) {
            prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Task Period %d", period);
        } else {
            prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "Scheduler error: %d", result);
        }
    } else {
        prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Usage: get_period [action ID]");
    }
}
...
void cmd_set_period(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 2) {
        ...
        if (period == -1) {
            log_str(ERROR, LOG_RTC_GENERAL, "Invalid period: %s", (char*)arg);
            return;
        }
        ...
        prompt_cmd_response(INFO, LOG_PRINT_GENERAL, "period changed %d, %d", action_id, period);
    } else {
        prompt_cmd_response(ERROR, LOG_RTC_GENERAL, "Usage: set_period [action ID] [new interval]");
    }
}
...
void cmd_erase_nvct(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 1) {
        ...
        if (table_index == -1) {
            log_str(ERROR, LOG_SETTINGS, "Invalid table index: %s", (char*)arg);
            return;
        }
        ...
        switch (err) {
            case NVCT_SUCCESS:
                prompt_cmd_response(INFO, LOG_SETTINGS, "NVCT %d erased", table_index);
                break;
            case NVCT_TABLE_INDEX_ERROR:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "Table %d invalid", table_index);
                break;
            case NVCT_FAILED_TO_WRITE_TO_MRAM_ERROR:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "MRAM error");
                break;
            default:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "Unknown error: %d", err);
                break;
        }
    } else {
        prompt_cmd_response(ERROR, LOG_SETTINGS, "Usage: erase_nvct [table index]");
    }
}
...
void cmd_provision_nvct(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 1) {
        ...
        switch (err) {
            case SETTING_OK:
                prompt_cmd_response(INFO, LOG_SETTINGS, "Provision OK %d", table_index);
                break;
            case SETTING_NVCT_UPDATE_ERR:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "NVCT failed");
                break;
            case INVALID_SETTING_NAME:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "No fw_version!");
                break;
            default:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "Unknown error: %d", err);
                break;
...
void cmd_get_setting(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 1) {
        ...
        switch (err) {
            case SETTING_OK:
                prompt_cmd_response(INFO, LOG_SETTINGS, "%s %d (%d)", (const char*)arg, val, nvct_info);
                break;
            case INVALID_SETTING_NAME:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s not found", (const char*)arg);
                break;
            default:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "Unknown error: %d (%d)", err, nvct_info);
                break;
        }
    } else {
        prompt_cmd_response(ERROR, LOG_SETTINGS, "Usage: get_setting [name]");
    }
}
...
void cmd_set_setting(uint32_t arg_len, void* arg) {
    ...
    if (num_args == 2) {
        ...
        if (val == -1) {
            log_str(ERROR, LOG_SETTINGS, "Invalid val: %s", (char*)arg);
            return;
        }
        ...
        switch (err) {
            case SETTING_OK:
                prompt_cmd_response(INFO, LOG_SETTINGS, "%s %d OK", (const char*)arg, val);
                break;
            case INVALID_SETTING_NAME:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s not found", (const char*)arg);
                break;
            case INVALID_SETTING_VALUE:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s invalid val", (const char*)arg);
                break;
            case SETTING_CALLBACK_FAILED:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s callback failed", (const char*)arg);
                break;
            case SETTING_IMMUTABLE:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s immutable", (const char*)arg);
                break;
            case SETTING_NVCT_UPDATE_ERR:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "%s NVCT err", (const char*)arg);
                break;
            default:
                prompt_cmd_response(ERROR, LOG_SETTINGS, "Unknown error: %d", err);
                break;
        ...
        prompt_cmd_response(ERROR, LOG_SETTINGS, "set_setting [name] [value]");
    }
}
...
static void prompt_cmd_response(log_level_t lvl, log_identifier_t id, char* format, ...) {
    ...
    va_end(va);
    log_str(lvl, id, "%s", last_logged.string);
}
