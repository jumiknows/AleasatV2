...
class OBCCmdSysMsgHeader(packet.Packet):
    ...
    def __init__(self, seq_num: int = None, cmd_id: int = None, date_time: obc_time.OBCDateTime = None, flags: int = None, resp: bool = False, data_len: int = None):
    ...
    def deserialize(self, data: bytes) -> "OBCCmdSysMsgHeader":
        ...
        return OBCCmdSysMsgHeader(seq_num, cmd_id, obc_time.OBCDateTime.from_timestamp(timestamp), flags, resp, data_len)
...
class OBCAppCmdSysDest(routing_impl.ProtocolPacketDest[Union[OBCCmdSysMsgHeader, packet.RawPacket], packet.RawPacket]):
