...
class OBCSerialProtocol:
    ...
    def __init__(self, log_specs: log_sys.log_spec.OBCLogGroupSpecs):
        ...
        self._rx_app_log_protocol = app_log.OBCAppLogRX(log_specs)
        ...
        self._rx_app_resp_raw     = routing_impl.ProtocolPacketSource[          #                                  |                |                 
                                        ...
                                        self._rx_app_resp_bridge                #                                  |                |                 
                                                                                ...
                                                                                #    .      .      .               |                |                 
        self._rx_app_resp         = app_cmd_sys.OBCAppCmdSysSource(             #     .     .     .                |      ----------------------      
                                        self._rx_app_resp_raw                   #      .    .    .                 |      | OBCAppCmdSysSource |      
                                    )                                           #       \   |   /                  |      ----------------------      
                                                                                #        \  |  /                   /                |                 
        self._rx_app_log_multi = routing_impl.MultiPacketDest[app_log.OBCLog]() #         \ | /                   /         OBCCmdSysMsgHeader        
                                                                                #         OBCLog                 /                  or                
        self._rx_app_log          = routing_impl.ProtocolPacketDest[            #           |                   /                 bytes               
                                        datalink.OBCSerialDatagram,             #           |                  /                                      
                                        app_log.OBCLog                          #            \                /                                       
                                    ](                                          #       ---------------      /                                        
                                        self._rx_app_log_protocol,              #       | OBCAppLogRX |     /                                         
        ...
        self._tx_datalink = routing_impl.ProtocolPacketDest[                    #                |                            |                       
                                ...
                                self._tx_bridge                                 #                |                                                    
                                                                                ...
                                                                                #            RawPacket                                                
        self._tx_app_cmd  = app_cmd_sys.OBCAppCmdSysDest(                       #                |                                                    
    ...
    def update_log_specs(self, log_specs: log_sys.log_spec.OBCLogGroupSpecs):
        ...
        """
        self._rx_app_log_protocol.update_log_specs(log_specs)

    def add_log_listener(self, queue_size: int = 0) -> routing_impl.QueuePacketBridge[app_log.OBCLog]:
    ...
    def remove_log_listener(self, listener: routing_impl.QueuePacketBridge[app_log.OBCLog]):
    ...
    def send_cmd_recv_resp(self, cmd: cmd_sys.cmd.OBCCmd, timeout: float = None) -> cmd_sys.resp.OBCResponse:
        ...
        """
        # Send Command
        self.send_cmd(cmd.cmd_sys_spec, *cmd.args, date_time=cmd.date_time, timeout=timeout)

        if cmd.cmd_sys_spec.resp or (not cmd.date_time.is_immediate):
            ...
            if resp_header.cmd_id != cmd.cmd_sys_spec.id:
                raise exc.OBCCmdSysResponseError(f"Command ID mismatch. Expected: {cmd.cmd_sys_spec.id}. Received: {resp_header.cmd_id}")

            if resp_header.date_time != cmd.date_time:
                raise exc.OBCCmdSysResponseError(f"Date/time mismatch. Expected: {cmd.date_time}. Received: {resp_header.date_time}")
            ...
            return cmd_sys.resp.OBCResponse.create(cmd.cmd_sys_spec, cmd.inst_id, resp_header.date_time, resp_header.flags, resp_data, ignore_fields=ignore_fields)

        return None

    # Send Command -------------------------------------------------------------------------------------

    def send_cmd(self, cmd_sys_spec: cmd_sys.spec.OBCCmdSysSpec, *args, date_time: obc_time.OBCDateTime = obc_time.IMMEDIATE, timeout: float = None) -> app_cmd_sys.OBCCmdSysMsgHeader:
        ...
        header = app_cmd_sys.OBCCmdSysMsgHeader(self._cmd_seq_num, cmd_sys_spec.id, date_time, 0, False, len(data))
