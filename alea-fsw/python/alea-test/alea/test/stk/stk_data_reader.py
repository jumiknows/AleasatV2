...
class STKDataReader:
    ...
    def __init__(self, data_file_path: Path, ic_file_path: Path):
        self.data_df: pd.DataFrame = pd.read_csv(data_file_path)

        with open(ic_file_path) as ic_file:
            self.ic_dict:dict = json.load(ic_file)
    ...
    @staticmethod
    def data_file_path(filename) -> contextlib.AbstractContextManager[Path, bool | None]:
        return as_file(files(stk_data).joinpath(filename))
