...
cmd_sys_resp_code_t cmd_impl_GPS_CONFIGURE_NMEA_MESSGES(const cmd_sys_cmd_t* cmd, cmd_GPS_CONFIGURE_NMEA_MESSGES_args_t* args) {
    gps_err_t err = gps_configure_nmea_messages(args->gga_interval, args->gsa_interval, args->gsv_interval, args->gll_interval, args->rmc_interval, args->vtg_interval, args->zda_interval);
    return gps_to_cmd_sys_resp(err);
}

cmd_sys_resp_code_t cmd_impl_GPS_QUERY_NMEA_MESSGES(const cmd_sys_cmd_t* cmd, cmd_GPS_QUERY_NMEA_MESSGES_resp_t* resp) {
    gps_err_t err = gps_query_nmea_messages(&resp->gga_interval, &resp->gsa_interval, &resp->gsv_interval, &resp->gll_interval, &resp->rmc_interval, &resp->vtg_interval, &resp->zda_interval);
    return gps_to_cmd_sys_resp(err);
}
