...
static void obc_main_task(void *pvParameters) {
    ...
    rtc_create_infra();
    filesystem_create_infra();
    telem_exec_create_infra();
    telem_create_infra();
    gps_create_infra();
    ...
    // Filesystem initialization requires flash
    flash_init();
    fs_init();

    gps_init();

    filesystem_start_task();

    // Telemetry collection task is started after all others to ensure all other hardware tasks are available
    telem_exec_start_task();
    telem_collect_start_task();
