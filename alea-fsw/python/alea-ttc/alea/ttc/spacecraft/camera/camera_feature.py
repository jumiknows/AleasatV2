...
from alea.ttc.util.data_utils import get_byte

from . import OV5642

class CameraFeature(ttc_base.TTCBase):
    """Implementation of camera features of the OBC
    """

    def camera_init(self, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_CAM_INIT", timeout=timeout)

    def camera_capture(self, timeout: float = DEFAULT_CMD_TIMEOUT, progress_callback: Callable[[data_utils.DataProgress], None] = None) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_CAM_CAPTURE", timeout=timeout, progress_callback=progress_callback)

    def camera_write_sensor_reg(self, addr: int, data: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_CAM_WR_SREG", addr, data, timeout=timeout)

    def camera_read_sensor_reg(self, addr: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_CAM_RD_SREG", addr, timeout=timeout)

    def camera_write_sensor_regs(self, regs: OV5642.RegisterSet, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        return self.send_obc_cmd("TEST_CAM_WR_SREGS", regs.serialize(), timeout=timeout)
    ...
    def camera_set_resolution(self,
                          ...
                          timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        # Read ISP_CONTROL_01
        ...
        isp_control_01_data = resp.data["data"]
    ...
    def camera_set_white_balance_manual(self, gain_red: int, gain_green: int, gain_blue: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        regs = OV5642.RegisterSet([
    ...
    def camera_set_white_balance_auto_simple(self, top_limit: int, bottom_limit: int, timeout: float = DEFAULT_CMD_TIMEOUT) -> cmd_sys.OBCResponse:
        regs = OV5642.RegisterSet([
