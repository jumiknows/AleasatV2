...
void watchdog_reset(void) {
    ...
    log_str(INFO, WD_LOG, true, "Suspending WDPT");
...
void vWatchdogPetTask(void* pvParameters) {
    ...
    while (1) {
        if (tasks_statuses_valid()) {
            log_str(DEBUG, WD_LOG, false, "Petting watchdog");
            pet_watchdog();
        } else {
            log_str(ERROR, WD_LOG, true, "One or more tasks has UNKNOWN");
        }
        vTaskDelay(pdMS_TO_TICKS(TASK_PERIOD_MS(pvParameters)));
    }
}
...
bool set_task_status(task_id_t task_id, watchdog_task_status_t status) {
    if (xSemaphoreTake(watchdog_mutex, pdMS_TO_TICKS(WD_MUTEX_WAIT_MS)) == pdTRUE) {
        ...
        log_str(ERROR, WD_LOG, true, "Failed to take watchdog mutex");
        return false;
    }
}
    ...
    for (i = 0; i < number_of_tasks(); i++) {
        if (get_task_handle(i) != xWatchdogPetTaskHandle) {
            ...
            if (!got_mutex) {
                ...
                log_str(ERROR, WD_LOG, "%s did not clear UNKNOWN", task_name);
                is_correct = false;
