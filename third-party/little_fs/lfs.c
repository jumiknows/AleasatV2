...
int lfs_remove(lfs_t *lfs, const char *path) {
    ...
    if (lfs_tag_type3(tag) == LFS_TYPE_DIR) {
       ...
       /* AZ: Commenting this out so that we can delete a whole directory without
		* having to go in and delete every file first. This seems to be an acceptable
		* change according to: https://github.com/ARMmbed/littlefs/issues/43
