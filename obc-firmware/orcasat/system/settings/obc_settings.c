...
void update_settings_from_nvct(void) {
    ...
    if (xSemaphoreTakeRecursive(settings_mutex, portMAX_DELAY)) {
        ...
        if (nvct_err == NVCT_SUCCESS) {
                ...
                if (settings[i].nvct == USE_NVCT) {
                    ...
                    if (get_nvct_err == NVCT_SUCCESS) {
                        ...
                        if (get_nvct_err == NVCT_CRC_ERROR) {
                            log_str(ERROR, SETTINGS_LOG, true, "%s CRC err", settings[i].name);
                        } else {
                            log_str(ERROR, SETTINGS_LOG, true, "%s NVCT err: %d", settings[i].name, get_nvct_err);
                        }
            ...
            log_str(ERROR, SETTINGS_LOG, true, "No NVCT: %d", nvct_err);
        }
...
setting_err_t provision_new_settings_table(uint32_t table_index) {
    ...
    if (ret == SETTING_OK) {
        ...
        // Handle errors.
        if (nvct_err != NVCT_SUCCESS) {
            log_str(ERROR, SETTINGS_LOG, true, "Provision NVCT err: %d %d", nvct_err, i);
        ...
        log_str(ERROR, SETTINGS_LOG, true, "No FW version");
    ...
    if (xSemaphoreTakeRecursive(settings_mutex, portMAX_DELAY)) {
        ...
        if (settings[idx].nvct != NOT_NVCT) {
            ...
            if (nvct_err == NVCT_SUCCESS) {
                ...
                nvct_ok = false;
                log_str(ERROR, SETTINGS_LOG, true, "%s NVCT err: %d", settings[idx].name, nvct_err);
            }
        }
...
static bool increment_revision_count(void) {
    ...
    if ((revision_count_index != -1) && (revision_count_index < num_settings)) {
        ...
        if (revision_setting->setting_value == 0) {
            log_str(INFO, SYS_LOG, true, "revision rollover");
        }
        ...
        if (nvct_err == NVCT_SUCCESS) {
            ...
            log_str(ERROR, SETTINGS_LOG, true, "Revision %d NVCT err: %d", revision_setting->setting_value, nvct_err);
            ok = false;
        }
    }
    return ok;
