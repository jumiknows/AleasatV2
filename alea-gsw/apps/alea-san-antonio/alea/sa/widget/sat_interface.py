...
class SatInterface(QtWidgets.QWidget, sat_interface_ui.Ui_SatInterface, ttcqt.OBCTTCInterfaceProvider):
    ...
    def _refresh_devices(self) -> None:
        ...
        for port in ports:
            ordered_ports.append(port.device)
            try:
                if int(port.location[-1]) == 0 and (port.vid == XDS110_VID or port.vid == FTDI_VID):
                    best_port = port.device
            except:
                print("Error: failed to grab interface number (MacOS)")

        if best_port:
            ordered_ports.remove(best_port)
            ordered_ports.insert(0, best_port)

        self._ports = ordered_ports
