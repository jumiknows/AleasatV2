...
/**
 * @file timers.c
 * @brief System timekeeping and clock
 *
 * Access to 16 or 32 bit variables is not atomic on 8 bit CPUs and should be protected by disabling interrupts (SDCC).
 * (i.e. the processor needs more than one instruction for the access and could be interrupted while accessing the variable
 *
...
/**
 * @brief Initialize timers to be used
 */
...
/**
 * @brief Get system uptime (since boot)
 *
 * @pre Initialize the timers
 * @param[in] t The Address of time struct in XDATA (seconds, nanoseconds)
 */
void timers_get_time(__xdata timespec_t *t) {
    ...
    // It is possible to read the 16-bit counter value through
    // the two 8-bit SFRs (T1CNTL and T1CNTH) which is what
    // we do here. T1CNTL shall always be read first
    // before reading T1CNTH (pg 112/249 of CC1110 datasheet)
    t_fine = T1CNTL;
    // Set the high byte
    t_fine |= T1CNTH << 8;
...
/**
 * @brief Updates seconds in time structure
 * @pre Initialize the timers
 */
 ...
 *
 * @pre Initialize the timers
 * @param[in] t The Read only address of time struct in XDATA (seconds, nanoseconds)
 */
...
/**
 * @brief RF Timer interrupt service routine
