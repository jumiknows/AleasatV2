...
comms_err_t comms_flash_image(comms_app_image_page_t* pages, uint8_t num_pages) {
    ...
    if (num_pages == 0) {
        log_str(ERROR, COMMS_LOG, false, "Comms flsh no pages");
        return COMMS_FLASH_FAIL;
    }

    err = comms_enter_bootloader_once();
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, COMMS_LOG, false, "Comms flsh btldr err %d", err);
        return COMMS_FLASH_FAIL;
    }

    err = comms_attempt_flash(pages, num_pages);
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, COMMS_LOG, false, "Comms flsh app err %d", err);
        return COMMS_FLASH_FAIL;
    }

    err = comms_wait_for_cmd(&app_start_match_spec, COMMS_FLASH_APP_START_TIMEOUT_MS);
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, COMMS_LOG, false, "Comms flsh start err %d", err);
        return err;
    }
