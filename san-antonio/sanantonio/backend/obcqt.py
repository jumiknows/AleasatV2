...
class OBCQTRequest:
    ...
    def __init__(self, func: Callable[[obcpy.obc.OBC], Any], callback: Callable[[Any], None] = None):
        self._func = func
        self._callback = callback

    @property
    def func(self) -> Callable[[obcpy.obc.OBC], Any]:
        return self._func
...
class OBCWorker(QtCore.QObject):
    ...
    obc_error = QtCore.pyqtSignal(exc.OBCError)
...
class OBCQTThread(QtCore.QObject):
    ...
    def __init__(self, name: str, obc: obcpy.obc.OBC, worker_cls: Type[OBCWorker], run_continuous: bool = True, **kwargs):
    ...
    def start(self):
        ...
        if not self.started:
            ...
            if self._run_continuous:
                ...
                self._worker.obc_error.connect(self.obc_error)
...
class OBCQT(QtCore.QObject):
    ...
    def add_event_listener(self, listener: obc_base.OBCEventListener):
        """Wrapper for OBCBase.add_event_listener(...)
        """
        self._obc.add_event_listener(listener)

    def remove_event_listener(self, listener: obc_base.OBCEventListener):
        """Wrapper for OBCBase.remove_event_listener(...)
        """
