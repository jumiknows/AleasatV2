...
from sanantonio.ui.control_panels import eps_graph_panel_ui
...
class EpsGraphPanel(QtWidgets.QWidget, eps_graph_panel_ui.Ui_EpsGraphPanel):
    ...
    def handle_measure_clicked(self):
        if self._timer.isActive():
            ...
            if self.obc.connected:
                ...
                console_utils.print_err("Cannot start EPS measurement - not connected to OBC")
    ...
    def _measure_callback(self, resp: Union[cmd_sys.resp.OBCResponse, exc.OBCError]):
        if not obc_utils.check_cmd_sys_resp(resp, "eps_err"):
            return

        if resp.data["eps_err"] != 0:
            console_utils.print_err(f"EPS error during measurement: {resp.data['eps_err']}")
            return
