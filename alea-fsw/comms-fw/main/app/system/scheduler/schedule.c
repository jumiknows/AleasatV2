...
__xdata uint32_t auto_reboot_time;
__xdata bool reboot_scheduled;

/**
 * @brief Intialize auto reboot feature
 * @pre Enable/disable this feature in board_defaults.h
 */
...
/**
 * @brief Postphone a scheduled reboot
 * @pre Schedule_init must be called
 * @param[in] postpone The time to postpone by time [seconds]
 */
uint8_t schedule_postpone_reboot(uint32_t postpone) {
    ...
    #endif
    auto_reboot_time = uptime + postpone;
    return SCHEDULE_REBOOT_POSTPONED;
}

/**
 * @brief Main radio thread that main radio process's follow
 * @pre Call schedule_init
 * @warning TIMER_COUNT_PERIOD will change telemetry collection loop rate
 *  and potentially give garbage samples
 */
void schedule_handle_events(void) {
    // Reboot if our uptime is too high
    if (auto_reboot_time != 0 && uptime >= auto_reboot_time) {
