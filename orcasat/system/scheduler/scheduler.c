...
void print_schedule(void) {
    ...
    for (i = 0; i < MAX_NUM_ACTIONS; i++) {
        ...
        log_str(INFO, SCHED_LOG, false, "%d %d %d %s", i, action_info_header->id, action_info_header->submission_time.epoch, submission_source_strings[action_info_header->source]);

        // Third line - schedule info: number, ID, scheduled epoch, interval
        log_str(INFO, SCHED_LOG, false, "%d %d %d %d", i, action_info_header->id, action_info_header->start_at.epoch, action_info_header->interval);
    }
}
...
static void vSchedulerTask(void* pvParameters) {
    ...
    for (;;) {
        ...
        if (errcode != RTC_NO_ERR) {
            log_str_no_time(ERROR, SCHEDULER_FAILED_TO_READ_RTC_CURRENT_TIME, true, "rtc err: %d", errcode);
        } else {
            xSemaphoreTake(actions_mutex, portMAX_DELAY);
