...
// From SDCC variables declared with this storage
// class "__code" will be placed in the code memory.
// We mask the channel bits so set up the extra channel
// select of ADCCON3
...
// Use the channel index to specify which
// ADC input to sample. It is accessed by
// the ISR so keep it volatile
static volatile uint8_t adc_channel_index;

/**
 * @brief intializes ADC
 *
 * @warning ADCCFG = ADCCFG_CONFIG overrides any Px_DIR settings
 */
void adc_init(void) {
    // Enable ADC inputs
...
/**
 * @brief Starts ADC sample
 * @pre ADC must first be initialized
 */
...
/**
 * @brief Wait for the sample ready bit to be set by the interrupt
 * @pre ADC must first be initialized
 */
void adc_wait(void) {
