...
comms_err_t comms_send_cmd(
    ...
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms send arg err %d", err);
        return err;
    }
    ...
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms send tx err %d", mibspi_ret);
        return err;
    }
...
comms_err_t comms_send_recv_cmd(
    ...
    err = comms_build_buffer(false, dest_hwid, comms_seqnum, cmd_num, cmd_data, cmd_data_len, buf);
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc arg err %d", err);
        return err;
    }
    ...
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc parm err 1 %d", err);
        return err;
    }
    ...
    do {
        ...
        // check comms_mibspi_tx result
        if (err != COMMS_SUCCESS) {
            log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc tx err %d", mibspi_ret);
            break;
        }
    ...
    if (err2 != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc parm err 2 %d", err2);
        return err2;
    }
    ...
    if ((uxBits & COMMS_RX_EVENT_UNBLOCK_WAITER_BIT) == 0) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc evt t-out");
...
comms_err_t comms_wait_for_cmd(
    ...
    if (err != COMMS_SUCCESS) {
        log_str(ERROR, LOG_COMMS_GENERAL, "Comms sdrc parm err 2 %d", err);
        return err;
    }
