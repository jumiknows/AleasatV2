...
cmd_sys_err_t cmd_sys_begin_response(const cmd_sys_cmd_t *cmd, cmd_sys_resp_code_t resp_code, uint32_t resp_data_len) {
    ...
    data_fmt_u32_to_arr_be(cmd->header.timestamp, &buf[idx]);
    idx += sizeof(cmd->header.timestamp);
    data_fmt_u16_to_arr_be(cmd->header.flags, &buf[idx]);
    idx += sizeof(cmd->header.flags);
    data_fmt_u32_to_arr_be((resp_data_len | CMD_SYS_RESPONSE_BIT), &buf[idx]);
    idx += sizeof(resp_data_len);
