...
class PyStructDataField(data_field.DataField):
    ...
    def encode(self, value: Any) -> bytes:
        if self.is_array:
            if not isinstance(value, abc.Iterable):
                raise data_field.DataFieldEncodeError(f"{value} is not an Iterable even though the field is an array", self)
            if len(value) != self.array_len:
                raise data_field.DataFieldEncodeError(f"{value} has length {len(value)} while the field expects {self.array_len} elements", self)
            ...
            try:
                ...
                raise data_field.DataFieldEncodeError(f"Error packing {value}: {e}", self)
        else:
    ...
    def decode(self, data: bytes) -> Any:
        try:
            ...
            raise data_field.DataFieldDecodeError(f"Error unpacking {data}: {str(e)}", self)
    ...
    def pre_encode(self, value: Any) -> int:
        ...
        if isinstance(value, str):
            try:
                ...
                raise data_field.DataFieldEncodeError(f"Failed to encode string as integer: \"{value}\"", self)
        ...
        if not isinstance(value, accepted_types):
            raise data_field.DataFieldEncodeError(f"{value} has invalid data type: {type(value)}", self)
    ...
    def pre_encode(self, value: Any) -> float:
        ...
        if isinstance(value, str):
            try:
                ...
                raise data_field.DataFieldEncodeError(f"Failed to encode string as float: \"{value}\"", self)
        # Handle ints
    ...
    def pre_encode(self, value: Any) -> bool:
        if isinstance(value, str):
            ...
            if value == "true":
                ...
                raise data_field.DataFieldEncodeError(f"Invalid Bool string: \"{value}\"", self)
...
class DateTime(U32):
    ...
    def pre_encode(self, value: Any) -> int:
        if isinstance(value, str):
            try:
                value = alea_time.ALEADateTime.from_string(value)
            except ValueError:
                raise data_field.DataFieldEncodeError(f"Invalid DateTime string: \"{value}\"", self)
        elif isinstance(value, int):
            value = alea_time.ALEADateTime.from_timestamp(value)

        if not isinstance(value, alea_time.ALEADateTime):
            raise data_field.DataFieldEncodeError(f"{value} has invalid data type: {type(value)}", self)

        return value.to_timestamp()

    def post_decode(self, value: int) -> alea_time.ALEADateTime:
        return alea_time.ALEADateTime.from_timestamp(value)
    ...
    def encode(self, value: Any) -> bytes:
        if isinstance(value, str):
            try:
                ...
                raise data_field.DataFieldEncodeError(f"Failed to encode string as ASCII: \"{value}\"", self)

        if isinstance(value, bytearray):
            value = bytes(value)

        if not isinstance(value, bytes):
            raise data_field.DataFieldEncodeError(f"{str(value)} has invalid data type: {type(value)}", self)
...
class String(Bytes):
    ...
    def decode(self, data: bytes) -> Any:
        try:
            ...
            raise data_field.DataFieldDecodeError(f"Failed to decode bytes as ASCII: {data}", self)
...
class DataFieldImpl(Enum):
    ...
    def create_data_field(cls, name: str, type_str: str) -> data_field.DataField:
        ...
        Raises:
            DataFieldError: If the type_str is invalid.
        ...
        if match is None:
            raise data_field.DataFieldError(f"Invalid type string (\"{type_str}\") for field: {name}")
